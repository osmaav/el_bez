/**
 * Тесты Firebase Authentication для LearningSection
 * 
 * @group Firebase
 * @section Learning
 * @module Auth
  ...
  describe('Firebase', () => {
    describe('Auth', () => {
      ...
      describe('Registration', () => {
        it('должен регистрировать нового пользователя с успешным результатом', async () => {
      ...
      describe('Login', () => {
        it('должен входить существующего пользователя с успешным результатом', async () => {
          const mockUserCredential = {
            user: {
              uid: 'test-user-id',
              email: 'test@example.com',
        ...
        it('должен обрабатывать ошибку при неподтверждённом email', async () => {
          vi.mocked(signInWithEmailAndPassword).mockRejectedValueOnce({
          ...
          await expect(
            signInWithEmailAndPassword(auth, 'unverified@example.com', 'password123')
          ).rejects.toThrow();
        });
      });

      describe('Logout', () => {
        it('должен выходить из системы успешно', async () => {
