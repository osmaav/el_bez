/**
 * Тесты Firebase Auth для ExamSection
 * 
 * @group Firebase
 * @section Exam
 * @module Auth
  ...
  describe('Firebase', () => {
    describe('Auth', () => {
      ...
      describe('Registration', () => {
        it('должен регистрировать нового пользователя для сохранения результатов экзамена', async () => {
          ...
          expect(result.user.email).toBe('exam@example.com');
        });
        ...
        it('должен обрабатывать ошибку при слабом пароле', async () => {
          ...
          await expect(
            createUserWithEmailAndPassword(auth, 'exam@example.com', '123')
          ).rejects.toThrow();
        });
      });

      describe('Login', () => {
        it('должен входить пользователя для доступа к экзамену', async () => {
          const mockUserCredential = {
            user: {
              uid: 'exam-user-id',
              email: 'exam@example.com',
        ...
        it('должен обрабатывать ошибку при неверном пароле', async () => {
          ...
          await expect(
            signInWithEmailAndPassword(auth, 'exam@example.com', 'wrongpassword')
          ).rejects.toThrow();
        });
