/**
 * Тесты Firebase Auth для TrainerSection
 * 
 * @group Firebase
 * @section Trainer
 * @module Auth
  ...
  describe('Firebase', () => {
    describe('Auth', () => {
      ...
      describe('Registration', () => {
        it('должен регистрировать нового пользователя для сохранения прогресса тренажёра', async () => {
          ...
          expect(result.user.email).toBe('trainer@example.com');
        });
        ...
        it('должен обрабатывать ошибку при слабом пароле', async () => {
          ...
          await expect(
            createUserWithEmailAndPassword(auth, 'trainer@example.com', '123')
          ).rejects.toThrow();
        });
      });

      describe('Login', () => {
        it('должен входить пользователя для доступа к тренажёру', async () => {
          const mockUserCredential = {
            user: {
              uid: 'trainer-user-id',
              email: 'trainer@example.com',
        ...
        it('должен обрабатывать ошибку при неверном пароле', async () => {
          ...
          await expect(
            signInWithEmailAndPassword(auth, 'trainer@example.com', 'wrongpassword')
          ).rejects.toThrow();
        });
